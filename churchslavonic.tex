...
\end{EN}
% XXX: What is a register name?

\begin{RU}
\subsection{\cs{cuNum}}
Команда печатает число кириллической цифирью.
Она принимает единственный аргумент. Аргументом может быть текст,
командная последовательность, или имя 
...
If your format uses \cs{cuDOW} or \cs{cuYEARAM}, and you specify a date according to the Julian, not Gregorian, calendar, you must use \cs{cuJulianDate} to
correctly format days of the week and year Anno Mundi.

The best practice is to always use \cs{cuDate} with Gregorian calendar dates and use \cs{cuJulianDate} with Julian calendar dates regardless
...
\end{EN}
% XXX: A Julian date is something else: https://en.wikipedia.org/wiki/Julian_day
% we need to avoid confusion here, so that the user knows he is inputting a date on the Julian calendar
% Not a Julian date
...
невозможными датами, например 32 апреля --- така дата будет отформатирована как 32 апреля. Так что команду \cs{cuDate} можно
...
Лучше всего всегда использовать \cs{cuDate} для форматирования дат по григорианскому календарю и всегда использовать \cs{cuJulianDate} для
...
\end{RU}
% XXX: тоже с русским термином юлианская дата.
% предлагаю заменить \cuJulianDate на \cuDateJulian
...
\item \cs{cuINDICTION} --- the indiction (a number from 1 to 15)
\end{itemize}
% XXX if you want, you can put a link here: https://en.wikipedia.org/wiki/Indiction

\footnotetext[1]{If your format uses this value, make sure that you format the date with the correct macro: you must use
\cs{cuDate} for dates on the Gregorian calendar and \cs{cuJulianDate} for dates on the Julian calendar.}
\end{EN}
...
\footnotetext[1]{Если ваш формат
    ...
    для дат по григорианскому календарю и \cs{cuJulianDate} для дат по юлианскому календарю.}
\end{RU}
