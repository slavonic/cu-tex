...
\begin{EN}
To customize the font and color of the margin mark use \cs{cuMarginMarkText},
which is applied to every subsequent mark printed.
For example, to make the marginal mark appear in red color, redefine \cs{cuMarginMarkText} like this:
\end{EN}
%
\begin{RU}
Шрифт и цвет пометы можно изменять переопределив команду \cs{cuMarginMarkText}, которая применяется к каждой последующей помете.
Например, если мы хотим, чтобы пометы печатались красным цветом, то достаточно переопределить \cs{cuMarginMarkText} 
следующим образом: 
\end{RU}
\begin{verbatim}
