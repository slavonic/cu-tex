...
\newfontfamily\russianfonttt[Scale=MatchLowercase]{FiraSlav}
...
\cuKinovar{Information:} The shade of red used by the \cs{cuKinovar} is declared in the package as a new color called \verb+kinovar+, and is set to (205, 8, 3) in the RGB color space, which has a hexadecimal representation of \verb+#CC0502+. The \verb+kinovar+ color may be redefined using the standard tools of the \verb+xcolor+ or \verb+luacolor+ packages, for example:
\end{EN}
...
\cuKinovar{Справка:} Оттенок красного цвета, используемый командой \cs{cuKinovar} объявляется в пакете новым цветом с названием \verb+kinovar+, и задан как (205, 8, 3) в RGB-цветовом пространстве, что равно \verb+#CC0502+ в шестнадцатиразрядном представлении. Цвет \verb+kinovar+ может быть переопределен используя стандартные макрокоманды пакетов \verb+xcolor+ или \verb+luacolor+, например:
\end{RU}

\begin{verbatim}
\definecolor{kinovar}{rgb}{0.00,1.0,0.0}
\end{verbatim}
...
\verb+invertColor+ & Inverts the colorization of the neumes, rendering the cinnabar marks in black and the black neumes in red (useful for typesetting Demestvenny Notation) \\
\hline
\end{tabular}
\end{center}
...
\verb+invertColor+ & Инвертирует раскраску крюков, выводя киноварные пометы в черном цвете, а невмы в красном (удобно для набора демественной нотации) \\
\hline
\end{tabular}
\end{center}
...
\begin{EN}
\noindent Example of Demestvenny Notation and Znamenny Notation side-by-side:
\end{EN}
%
\begin{RU}
\noindent Пример набора демественной и знаменной нотаций бок о бок:
\end{RU}

\begin{verbatim}
\cuKrukPara[invertColor,krukFont=\demestvo]{𜾧𜼾𜼆𜽝𜼐 \\ ла}
\cuKrukPara{𜽐𜼱𜼳𜼆 𜽽𜼳𜼅𜼣 \\ ла-~}
\end{verbatim}
...
\begin{churchslavonic}
\noindent
\cuKrukPara[invertColor,krukFont=\demestvo]{
𜾧𜼾𜼆𜽝𜼐
\\
ла
}
