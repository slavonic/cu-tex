...
\begin{EN}
\cuKinovar{Attention:} if you have older version of \pkg{fontspec} package installed on your system,
redefined underscore symbol can not be used in font names and font options in \pkg{fontspec} commands like
...
With the older \pkg{fontspec} this will cause errors. This problem exists in \texttt{texlive 2013} and in \pkg{fontspec v2.3c}.

We recommend to upgrade your \TeX\ distribution to at least \texttt{texlive 2015}. Alternatively you can selectively
upgrade \pkg{fontspec} package to version \texttt{v2.4c} or better.
...
Эта проблема присутствует в \texttt{texlive 2013} и в \pkg{fontspec v2.3c}.

Мы рекомендуем обновить систему \TeX\ до \texttt{texlive 2015} или свежее. Или же обновить только пакет \pkf{fontenc} до
...
Command \cs{cuMarginMark} is used to place a short text on the margin on the same level as the line where macro is placed.
Example:
\end{EN}
%
\begin{RU}
...
Команда \cs{cuMarginMark} используется для печати значка на полях, напротив строки где вызвана эта команда. Например:
Example:
\end{RU}
%
\begin{center}
...
The distance between the mark and the text is controlled by the value of \cs{cuMarginMarkSkip}. Default is:
\end{EN}
%
\begin{RU}
...
For example, to make margin mark appear in red color, redefine \cs{cuMarginMarkText} in the preamble of your
document like this:
\end{EN}
%
\begin{RU}
...
\begin{EN}
If you need to change fint/size/color just for a single mark, just do it with \cs{cuMarginMark}:
\end{EN}
